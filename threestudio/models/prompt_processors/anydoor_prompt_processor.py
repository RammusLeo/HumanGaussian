...
@threestudio.register("anydoor-prompt-processor")
class AnydoorPromptProcessor(PromptProcessor):
    @dataclass
    ...
    def spawn_func(pretrained_model_name_or_path, prompts, cache_dir):
        ...
        # identity_encoder = FrozenDinoV2Encoder().to(text_encoder.device)
        with torch.no_grad():
            tokens = tokenizer(
                prompts,
                padding="max_length",
            ...
            # identity_embeddings = identity_encoder(guidance_imgs.to(identity_encoder.device))
        for prompt, embedding in zip(prompts, text_embeddings):
            # embedding = torch.cat((embedding, identity_embedding),dim=0)
            torch.save(
                embedding,
                os.path.join(
                    cache_dir,
        ...
        # for prompt, embedding, identity_embedding in zip(prompts, text_embeddings,identity_embeddings):
        #     embedding = torch.cat((embedding, identity_embedding),dim=0)
        #     torch.save(
        #         embedding,
        #         os.path.join(
